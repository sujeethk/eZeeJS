 ...
 *
 */
logger.setupFileLogger = function setupFileLogger(_config) {
  config = _config;
...
logger.getLogOptions = function getLogOptions() {
  ...
  var configFileLogger = _config.fileLogger;

  if (!_.has(_config, 'fileLogger.directoryPath') || !_.has(_config, 'fileLogger.fileName')) {
...
logger.getLogFormat = function getLogFormat() {
  var format = config && config.format ? config.format.toString() : 'combined';
